	...
	if (is_admin()){
		...
		if(!function_exists('aisis_load_admin_js')){
		   function aisis_load_admin_js(){
			   ...
			   wp_enqueue_script( 'admin-js', get_template_directory_uri() . '/AisisCore/AdminPanel/Modules/required/AdminPanelJS.js', array('jquery','media-upload','thickbox'), false, true );
		...
		if(!function_exists('aisis_add_settings_page')){
			function aisis_add_settings_page() {
				...
				add_submenu_page('aisis_options', __('Css Editor', 'aisis'), __('CSS Editor', 'aisis'), 'edit_themes', 'aisis-css-editor', array('aisis_site_options', 'aisis_admin_panel')); #wp
				add_submenu_page('aisis_options', __('PHP editor', 'aisis'), __('PHP Editor', 'aisis'), 'edit_themes', 'aisis-php-editor', array('aisis_site_options', 'aisis_admin_panel')); #wp
				add_submenu_page('aisis_options', __('JS Ediotr', 'aisis'), __('JS Ediotr', 'aisis'), 'edit_themes', 'aisis-js-editor', array('aisis_site_options', 'aisis_admin_panel')); #wp
			}
		}
		...
		if(isset($_GET['page']) && $_GET['page'] == 'aisis_options' || isset($_GET['page']) && $_GET['page'] == 'aisis-css-editor' || isset($_GET['page']) && $_GET['page'] == 'aisis-php-editor' 
			|| isset($_GET['page']) && $_GET['page'] == 'aisis-js-editor' ){
